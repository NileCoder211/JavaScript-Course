...
import {BookSystem} from "./services/LibraryService"
...
let currentUser = new Member("Khorado Riak", "khorado@email.com");
const library = new BookSystem();

userSwitcher.addEventListener("change", (e)=>{
    const selected = e.target.value;
    currentUser = selected === "admin" ? new Admin("Khor Koang", "khor@gmail.com") : 
    new Member("Nyasunday Deng", "nyasunday@email.com")
...
function renderBooks(){
   ...
   library.getAllBooks().forEach((book)=>{
    ...
    if (currentUser.getRole() === "Member" && book.isAvailable){
        controls = `<button class="bg-green-500 text-white px-3 py-1 rounded" 
        data-action="borrow" data-id="${book.id}">Borrow</button>`;
    }else if(currentUser.getRole() === "Admin"){
        controls = `<span class="test-sm text-gray-500">${book.isAvailable? "Available" : "Borrowed"}</span>`
    }
      li.innerHTML = `
      <div>
